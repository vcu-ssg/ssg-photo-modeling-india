export async function loadPLYViewer(containerId, plyPath) {
  ...
  // === NEW: restrict keyboard to only when mouse is inside ===
  let isPointerInside = false;
  container.addEventListener('mouseenter', () => { isPointerInside = true; });
  container.addEventListener('mouseleave', () => { isPointerInside = false; });
  ...
  function animate() {
    ...
    if (modelGroup) {
      ...
      if (autoShowcase) {
        modelGroup.rotation.x += 0.025;
        modelGroup.rotation.y += 0.025 * 0.7;
        modelGroup.rotation.z += 0.025 * 0.3;
      }
